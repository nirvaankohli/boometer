...
class from_emotion_scores:
    ...
    def __init__(self, emotion_scores: dict = None, debug: bool = False):
    ...
    def set_emotion_scores(self, emotion_scores: dict):
        ...
        self.print_debug(f"Setting emotion scores: {emotion_scores}")
        self.emotion_scores = emotion_scores

    def get_emotion_scores(self) -> dict:

        self.print_debug(f"Getting emotion scores: {self.emotion_scores}")
        return self.emotion_scores

    def get_default_weights(self) -> dict:

        self.print_debug("Getting default weights.")
        default_weights = {
            "anger": 2.5,
            "disgust": 5.0,
            "fear": 10.0,
    ...
    def calculate_fear_score(self) -> float:
        ...
        try:
            ...
            self.print_debug("Calculating fear score.")
            fear_score = 0.0

            top_items = list(
                itertools.islice(
                    sorted(
                        self.emotion_scores.items(), key=lambda x: x[1], reverse=True
                    ),
                    5,
                )
            )

            self.print_debug(f"Top emotion scores considered: {top_items}")
            ...
            self.print_debug(
                f"Calculated fear score: {fear_score} using emotion scores: {emotion_scores}"
            )

        except Exception as e:

            self.print_debug(f"Error calculating fear score: {str(e)}")
            fear_score = 0.0
